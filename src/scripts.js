document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateKMapCellState(index, value) {
        ...
        kmap[index] = value;
        const cellElement = document.querySelector(`.kmap-cell[data-index="${index}"]`);
        if (cellElement) {
    ...
    kmapCells.forEach(cell => {
        cell.addEventListener('click', function() {
            ...
            if (selectedTool === TOOLS.SET_1 || selectedTool === TOOLS.SET_0 || selectedTool === TOOLS.SET_X) {
                const value = selectedTool === TOOLS.SET_1 ? 1 : (selectedTool === TOOLS.SET_0 ? 0 : TOOLS.SET_X);
                ...
                const isMintermModeValid = currentMode === MODES.MINTERMS && (cellValue === 1 || cellValue === TOOLS.SET_X);
                const isMaxtermModeValid = currentMode === MODES.MAXTERMS && (cellValue === 0 || cellValue === TOOLS.SET_X);
                ...
                if (isMintermModeValid || isMaxtermModeValid) {
                    ...
                    const validValues = currentMode === MODES.MINTERMS ? "1 or X" : "0 or X";
    ...
    Object.entries(toolButtons).forEach(([key, button]) => {
        ...
        if (key === 'clearKmap') {
            ...
            button.addEventListener('click', () => {
                const tool = button.id === 'setTo1' ? TOOLS.SET_1 :
                             button.id === 'setTo0' ? TOOLS.SET_0 :
                             button.id === 'setToX' ? TOOLS.SET_X :
                             button.id === 'selectTool' ? TOOLS.SELECT : null;
    ...
    modeRadios.forEach(input => {
        input.addEventListener('change', function() {
            ...
            Object.values(inputSections).forEach(section => section.classList.add(CSS_CLASSES.D_NONE));
    ...
    function updateModeUI() {
        ...
        actionButtons.generateFromSelection.disabled = !isGenerateFromSelectionEnabled;
        toolButtons.selectTool.disabled = !isSelectToolEnabled;

        // Update Button Styles
        actionButtons.generateFromSelection.classList.toggle(CSS_CLASSES.BTN_INFO, isGenerateFromSelectionEnabled);
        actionButtons.generateFromSelection.classList.toggle(CSS_CLASSES.BTN_SECONDARY, !isGenerateFromSelectionEnabled);

        toolButtons.selectTool.classList.toggle(CSS_CLASSES.TEXT_MUTED, !isSelectToolEnabled);
    ...
    actionButtons.generateFromSelection.addEventListener('click', function() {
        if (currentMode === MODES.MAXTERMS) {
            ...
            alert('Generate from Selection is currently only implemented for minterm (SOP) mode.');
            return;
        }
        ...
        const expression = generateExpressionFromCells(selectedIndices, currentMode); // Pass mode
    ...
    actionButtons.verifyExpression.addEventListener('click', function() {
        ...
        try {
            ...
            for (let i = 0; i < KMAP_SIZE; i++) {
                ...
                const variables = {
                    A: binary[0] === '1', B: binary[1] === '1', C: binary[2] === '1', D: binary[3] === '1',
                    "A'": binary[0] !== '1', "B'": binary[1] !== '1', "C'": binary[2] !== '1', "D'": binary[3] !== '1'
                };
            ...
            if (useAnywayBtn) {
                useAnywayBtn.addEventListener('click', () => {
    ...
    function addTruthTableClickListeners() {
        const outputCells = displayAreas.truthTableBody.querySelectorAll(`td.${CSS_CLASSES.TT_CLICKABLE}`);
    ...
    function handleTruthTableClick(event) {
        if (currentMode !== MODES.TRUTH_TABLE) {
            alert(`Switch to ${MODES.TRUTH_TABLE} mode to edit the truth table directly.`);
            return;
        }
    ...
    actionButtons.generateFromTruthTable.addEventListener('click', function() {
        if (currentMode !== MODES.TRUTH_TABLE) {
             alert(`This button is intended for use in ${MODES.TRUTH_TABLE} mode, but the K-Map is already updated live.`);
    ...
    actionButtons.resetTruthTable.addEventListener('click', function() {
        if (confirm('Are you sure you want to reset the entire K-Map and Truth Table to all zeros?')) {
    ...
    function solveKMap() {
        ...
        if (currentMode === MODES.MINTERMS) {
            ...
            if (minterms.length === 0 && dontCares.length === KMAP_SIZE) {
                ...
                groupsInfoHTML = 'No minterms (all zeros or don\'t cares resulting in 0)';
                ...
                const { essentialPIs, finalGroups } = selectEssentialPrimeImplicants(primeImplicants, minterms);

                // Generate expression from the final selected groups
                const finalExpressions = finalGroups.map(group => groupToExpressionSOP(group.cells));
                simplifiedExpression = finalExpressions.length > 0 ? finalExpressions.join(' + ') : '0'; // Should not be 0 if minterms exist

                // Generate detailed group info
                groupsInfoHTML = finalGroups.map((group, index) => {
                 ...
                 if (finalGroups.length === 0 && minterms.length > 0) {
                     ...
                     simplifiedExpression = minterms.map(m => groupToExpressionSOP([m])).join(' + ');
                     groupsInfoHTML = minterms.map(m => `Singleton Minterm: [${m}] → ${groupToExpressionSOP([m])}`).join('<br>');
                 }
            }
        } else if (currentMode === MODES.MAXTERMS) {
            // --- Product of Sums (POS) Logic ---
             if (maxterms.length === 0 && dontCares.length === KMAP_SIZE) {
                ...
                const { essentialPIs, finalGroups } = selectEssentialPrimeImplicants(primeImplicantsPOS, maxterms);

                // Generate POS expression (AND of OR terms)
                const finalExpressionsPOS = finalGroups.map(group => groupToExpressionPOS(group.cells));
                simplifiedExpression = finalExpressionsPOS.length > 0 ? finalExpressionsPOS.join(' · ') : '1'; // Default to 1 if no maxterms
                ...
                groupsInfoHTML = finalGroups.map((group, index) => {
                 ...
                 if (finalGroups.length === 0 && maxterms.length > 0) {
                     simplifiedExpression = maxterms.map(m => groupToExpressionPOS([m])).join(' · ');
                     groupsInfoHTML = maxterms.map(m => `Singleton Maxterm: [${m}] → ${groupToExpressionPOS([m])}`).join('<br>');
                 }
            }
        } else { // Truth Table Mode - Just display info, no simplification shown
    ...
    function findPrimeImplicants(targetTerms, dontCares) {
        ...
        for (const size of [16, 8, 4, 2, 1]) { // Include 16 and 1 for edge cases
             if (size === 16) { // Check for all 1s/0s case (handled in solveKMap, but check here too)
            ...
            for (const groupCells of potentialGroups) {
                ...
                const isValidGroup = groupCells.every(cell => allCoverableCells.has(cell));
                if (!isValidGroup) continue;

                // 2. Check if the group covers at least one target term (not just don't cares)
                const coversTargetTerm = groupCells.some(cell => targetTerms.includes(cell));
                ...
                const groupTargetTerms = groupCells.filter(cell => targetTerms.includes(cell));
                const isSubsumed = groupTargetTerms.every(term => coveredTargetTerms.has(term));
         ...
         const allCoveredTerms = new Set(primeImplicants.flatMap(pi => pi.cells.filter(cell => targetTerms.includes(cell))));
         targetTerms.forEach(term => {
             if (!allCoveredTerms.has(term)) {
                 const id = term.toString();
                 if (!primeImplicants.some(pi => pi.id === id)) { // Avoid duplicates if already added as size 1
        ...
        const finalPIs = primeImplicants.filter(pi => {
            return !primeImplicants.some(otherPI => {
                ...
                return pi.id !== otherPI.id && // Not comparing to itself
                       pi.size < otherPI.size && // otherPI must be larger
                       pi.cells.every(cell => otherPI.cells.includes(cell)); // All cells of pi must be in otherPI
            });
        });
    ...
    function selectEssentialPrimeImplicants(primeImplicants, targetTerms) {
        ...
        piChart.forEach((piIds, term) => {
            if (piIds.length === 1) {
                ...
                if (essentialPI && !essentialPIs.some(epi => epi.id === essentialId)) {
        ...
        if (remainingTargetTerms.size > 0) {
            ...
            const remainingPIs = primeImplicants.filter(pi => !essentialPIs.some(epi => epi.id === pi.id));
            ...
            while (remainingTargetTerms.size > 0 && remainingPIs.length > 0) {
                ...
                remainingPIs.forEach(pi => {
                    const coveredCount = pi.cells.filter(cell => remainingTargetTerms.has(cell)).length;
                    if (coveredCount > 0) {
                        ...
                        if (coveredCount > maxCoveredCount || (coveredCount === maxCoveredCount && pi.size > bestPIChoice?.size)) {
                ...
                if (bestPIChoice) {
                    ...
                    bestPIChoice.cells.forEach(cell => remainingTargetTerms.delete(cell));
                    // Remove the chosen PI from further consideration in this loop
                    const indexToRemove = remainingPIs.findIndex(pi => pi.id === bestPIChoice.id);
                    if (indexToRemove > -1) {
                    ...
                    console.error("Could not cover remaining terms:", Array.from(remainingTargetTerms));
                    break;
                }
            }
        }
        ...
        finalGroups = Array.from(new Map(finalGroups.map(g => [g.id, g])).values());
        ...
        finalGroups.sort((a, b) => {
             ...
             const exprA = (currentMode === MODES.MINTERMS) ? groupToExpressionSOP(a.cells) : groupToExpressionPOS(a.cells);
             const exprB = (currentMode === MODES.MINTERMS) ? groupToExpressionSOP(b.cells) : groupToExpressionPOS(b.cells);
    ...
    function getPotentialGroups(size) {
        ...
        // --- Groups of 4 ---
            ...
            // 2x2 Squares
            for (let r = 0; r < 3; r++) { // Row start (0, 4, 8)
                for (let c = 0; c < 3; c++) { // Col start (0, 1, 2)
                    const start = r * 4 + c;
                    ...
                    // Standard 2x2 block (no wrap)
                     ...
                     if (r === 2 && c < 3) { // Last standard row start
             ...
             // Rows
             groups.push([0, 1, 2, 3]); groups.push([4, 5, 6, 7]); groups.push([8, 9, 10, 11]); groups.push([12, 13, 14, 15]);
             // Columns
             groups.push([0, 4, 8, 12]); groups.push([1, 5, 9, 13]); groups.push([2, 6, 10, 14]); groups.push([3, 7, 11, 15]);
             // 2x2 Squares
             groups.push([0, 1, 4, 5]); groups.push([2, 3, 6, 7]); groups.push([8, 9, 12, 13]); groups.push([10, 11, 14, 15]);
             // Wraps
    ...
    function groupToExpressionPOS(group) {
        ...
        let expr = [];
        if (constantVars.A === '0') expr.push("A"); // 0 becomes uncomplemented
